...
public class RemarkController {
    ...
    public CommonResponse queryAll(RemarkEntity remarkFilter, Integer pageNum, Integer pageSize, Integer source) {
        log.info("客户端 获取所有的评论信息");

        // /queryAll?source=2
        if (source == 2 && this.user.getUserRight() == 1){
            // 如果是管理员后台,就不进行过滤
            response.setData(this.remarkService.queryAll(null, pageNum,pageSize));
            return response;
        }
        // 没有登录或者是一般用户的情况下，只能访问【审核通过】的评论
